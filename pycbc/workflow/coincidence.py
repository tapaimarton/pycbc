...
def setup_multiifo_interval_coinc(workflow, hdfbank, trig_files, stat_files,
    ...
    for veto_file, veto_name in zip(veto_files, veto_names):
        for i in range(factor):
            ...
            for j in range(len(trigfilepairs)):
                coinc_node_pairs = findcoinc_exe.create_node(trigfilepairs[j], hdfbank,
                                                   stat_files,
                                                   veto_file, veto_name,
                                                   group_str,
                                                   tags=[veto_name, str(i), str(j)])
                bg_files += coinc_node_pairs.output_files
                workflow.add_node(coinc_node_pairs)

    if len(workflow.ifos) == 3:
        trigfilepairs = list(itertools.combinations(trig_files,2))
        for veto_file, veto_name in zip(veto_files, veto_names):
            for i in range(factor):
                group_str = '%s/%s' % (i, factor)
                ...
                for j in range(len(trigfilepairs)):
                    coinc_node_pairs = findcoinc_exe.create_node(trigfilepairs[j], hdfbank,
                                                   ...
                                                   veto_file, veto_name,
                                                   group_str,
                                                   tags=[veto_name, str(i), str('detectorpair' + j)])
                    bg_files += coinc_node_pairs.output_files
                    workflow.add_node(coinc_node_pairs)
